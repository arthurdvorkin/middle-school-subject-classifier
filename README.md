...
for the list of subjects, as there would be too significant overlap between the 2 subjects, therefore, throwing off
...
I used synthetic generation as the data that needed to be generated here more or less simple and GPT could easily generate
a diverse and accurate dataset. I did have to stop generating at a point due to OPENAI rate limit for gpt-3.5-turbo.

### Post Processing

For all the data, I trimmed all data to be under 512 tokens long. All the splits were done in a way that sentences were
...
text such as current speaker indicator. I also made sure the dataset was balanced. As some of the subjects only had syntheticly
...
The validation set was used to validate during training. The test set was used to test model performance after training.
...
This is fine as models like DistilBERT tend to train better for classification when there are longer texts in the training set.
After digging around on the internet and doing an A/B training test, I confirmed that longer texts indeed perform better.
From my AB test, the accuracy only went up by around 5%. 
...
But I would like to note that some traditional ML constructs, primarily SVMs and TF-IDFs, have shown that, for fairly simple text classification problems 
such as the one I am working on here, performance is very similar to fine tuned pre-trained
transformer based models (check sources 1, 2, & 3 at the bottom of this readme).
...
The methodology I took to fine tuning was doing keeping things as simple as possible and reaching
...
possible way to train the model and see what results I get. The following two points led
me to think that the auto trainer would be sufficient for the needs of this problem.

1. The scope of the problem is narrow and the separation between categories is fairly large.
2.  The data, from looking at it, is fairly simple, well defined per category, and diverse

Given the above 2 points. I concluded that its more time efficient to forego writing the boiler plate
...
prevent the model from overfitting. Also used the precision per class and accuracy per class metrics
