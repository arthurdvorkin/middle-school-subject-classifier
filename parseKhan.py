...
class KhanParse:
    ...
    def get_transcript(self,path):
        ...
        try:
            python_button = driver.find_element("id","ka-videoPageTabs-tabbedpanel-tab-1-inner")
            python_button.click()
        except:
            try:
                python_button = driver.find_element("id","ka-videoPageTabs-tabbedpanel-tab-1")
                python_button.click()
            except:
                return
        ...
        first_sentence = ""
        for span in spans:
            if (span.text != "•" and span.text != "Current transcript segment: " and len(span.text) > 5):
        ...
        with open(self.file_path, 'a', newline='') as output_file:
            ...
            for output_string in output_strings:
                if output_string != "":
                    dict_writer.writerow({"subject": self.subject, "text": " ".join(texts)})
